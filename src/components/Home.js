...
import { eventData, upcomingSessions, featuredSpeakers } from './homeData';
...
const Home = () => {
    ...
    // Use data directly instead of fetching
    ...
    if (currentPage === 'calendar') {
        return <Schedule onNavigate={handleNavigation}/>;
    }

    if (currentPage === 'partners') {
        return <Sponsors onNavigate={handleNavigation}/>;
    }

    const { eventData: event, upcomingSessions: sessions, featuredSpeakers: speakers } = data;
        ...
        <div className="min-h-screen bg-white text-black font-bold">
            ...
            <section className="py-16 px-6 relative min-h-[600px] overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-12 gap-8 relative z-10">
                    {/* Left Side - Countdown */}
                    <div className="col-span-12 md:col-span-3 flex flex-col items-center md:items-start space-y-6 md:space-y-8">
                        {/* heading + countdown */}
                        ...
                        <button
                            ...
                            className="w-full md:w-auto bg-green-600 hover:bg-green-500 transition py-3 px-10 rounded-md text-xl font-black text-white text-center"
                        >
                            REGISTER
                        </button>
                        ...
                        <div className="text-center md:text-left text-gray-600 text-sm">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-2">
                                {['Keynote Speakers', 'Event Sponsors', 'Start-ups', 'Academia'].map(role => (
                                    <span
                                        key={role}
                                        className="px-2.5 py-1 rounded border border-gray-400 bg-gray-100 hover:border-green-500 transition whitespace-nowrap text-xs"
                                    >
                                        {role}
                                    </span>
                                ))}
            ...
            <section className="py-16 px-6 border-t border-gray-200">
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-center mb-12">
                        <div className="w-16 h-16 border border-gray-400 rounded-full flex items-center justify-center mr-4">
                        ...
                        {speakers.map((speaker, index) => (
                            <div key={speaker.id} className="mb-8">
                                <div className="w-full h-48 bg-gray-100 rounded-md mb-4 overflow-hidden relative">
                                    ...
                                    />
                                    <div className={`hidden w-full h-full absolute inset-0 ${index === 0 ? 'bg-gradient-to-b from-purple-600 to-green-600' : 'bg-gradient-to-b from-blue-600 to-green-600'} flex items-center justify-center`}>
                            ...
                            <div key={session.id} className="flex mb-8">
                                <div className="w-32 h-32 bg-gray-100 rounded-md mr-6 flex-shrink-0 overflow-hidden">
                                    <div className={`w-full h-full flex flex-col items-center justify-center relative overflow-hidden rounded-md ${
                                        ...
                                        {/* Main icon */}
                                        <div className="text-2xl font-bold text-white drop-shadow-lg transform group-hover:scale-110 transition-transform duration-300 mb-3">
            ...
            <section className="py-16 px-6 border-t border-gray-200">
                <div className="max-w-7xl mx-auto">
                    <h2 className="text-3xl font-black mb-8">Summit Sponsors</h2>
                    {sponsorsData.map(group => (
                        <div key={group.id} className="mb-8">
                            <h3 className="text-xl font-bold mb-4">{group.title}</h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                {group.sponsors.map(sponsor => (
                                    <div key={sponsor.id} className="flex flex-col items-center p-4 border border-gray-200 rounded-lg">
                                        <img src={sponsor.logo} alt={sponsor.name} className="w-16 h-16 object-contain mb-2" />
                                        <div className="text-sm font-bold text-center">{sponsor.name}</div>
                                        {sponsor.founder && <div className="text-xs text-gray-500 text-center">{sponsor.founder}</div>}
                                    </div>
                                ))}
                            </div>
                        </div>
